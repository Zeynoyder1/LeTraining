...
export default function CreateSessionScreen({ navigation }) {
  const createSession = useMutation(api.training.createSession);
  const openaiKeySet = useQuery(api.openai.openaiKeySet) ?? true;

  const [isAdvancedAnalysisEnabled, setIsAdvancedAnalysisEnabled] =
    useState(false);
  ...
  const toggleAdvancedAnalysis = () => {
    setIsAdvancedAnalysisEnabled(!isAdvancedAnalysisEnabled);
  };
  ...
  const createTrainingSession = async () => {
    ...
    if (!sessionTitle.trim()) {
      Alert.alert("Error", "Please enter a title for your training session");
      return;
    }
    if (!sessionNotes.trim()) {
      Alert.alert("Error", "Please enter notes for your training session");
      return;
    }
    ...
    try {
      const _sessionId = await createSession({
        title: sessionTitle.trim(),
        notes: sessionNotes.trim(),
        isAnalyzed: isAdvancedAnalysisEnabled,
      });
      navigation.navigate("TrainingDashboardScreen");
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.underHeaderContainer}>
        ...
        <Text style={styles.title}>Create a New Training Session</Text>
        <TouchableOpacity>
          <Image
            style={styles.arrowBack}
      ...
      >
        <View style={styles.inputContainer}>
          ...
          <TextInput
            value={sessionTitle}
          onChangeText={(val: string) => setSessionTitle(val)}
          ...
          <TextInput
            value={sessionNotes}
          onChangeText={(val: string) => setSessionNotes(val)}
        ...
        <View style={styles.advancedSummarizationContainer}>
          <View style={styles.advancedSummarizationCheckboxContainer}>
            <TouchableOpacity
              onPress={toggleAdvancedAnalysis}
            ...
            <Text style={styles.advancedSummarizationText}>
              Advanced Analysis {openaiKeySet ? "" : " (Disabled)"}
            </Text>
          </View>
        ...
        >
          {isCreating ? (
            ...
            <>
              ...
              <Text style={styles.newNoteButtonText}>Create Training Session</Text>
            </>
          )}
        </TouchableOpacity>
      </Animated.View>
